...
export default function SponsorGallery() {
    ...
    if (images.length == 0) {
        return <main><div className="h-dvh">Loading...</div></main>
    }

    const img = (image: Sponsor) => {
        let result = <Image src={image.src} width={0} height={0} className="w-full" alt={image.alt} fill={false} />

        if (image.href !== null) {
            result = <a href={image.href} target="_blank">{result}</a>
        }
